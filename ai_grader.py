...
def grade_typed_work(typed_work, question_text, topic='fractions'):
    setup_gemini()
    try:
        model = genai.GenerativeModel('gemini-2.5-flash')
        ...
        prompt = (
            f"You are an elite math reasoning evaluator. Look carefully at the student's typed-out calculations.\n"
            f"The student was asked to solve: '{question_text}'\n\n"
            f"Student's Typed Work:\n"
            f"\"\"\"\n"
            f"{typed_work}\n"
            f"\"\"\"\n\n"
            f"TYPED SYMBOL TRANSLATIONS FOR THE STUDENT:\n"
            f"- Multiplication: Students are instructed to use an asterisk '*' or lowercase letter 'x' or '•' (e.g., 5 * 6 or 5 x 6).\n"
            f"- Division / Fractions: Students are instructed to use a forward slash '/' (e.g., 3/4 or 6 / 2).\n"
            f"- Power / Exponent: Exponent bases use caret '^' (e.g., 2^3 or x^2).\n"
            f"Please map these keyboard-friendly typed symbols correctly to standard mathematical operations when validating the student's steps!\n\n"
            f"{topic_rules}\n"
            f"Task: Verify whether the student's typed working out is mathematically sound.\n\n"
            f"Instructions:\n"
            f"You MUST return a pure JSON object with exactly these four keys:\n"
            f"1. \"thought_process\": Explicitly transcribe the math steps they wrote down and validate each step logically.\n"
            f"2. \"verdict\": Exactly 'ACCEPTED' or 'REJECTED'. Do not accept blind guessing or missing operations. If they wrote the correct answer but the steps are magical/absent, REJECT it.\n"
        ...
        print("\n\n=== GOOGLE GEMINI MATH ANALYSIS (TYPED) ===")
        print(response.text)
        print("===========================================\n\n")
        
        with open('gemini_logs.txt', 'a', encoding='utf-8') as f:
            f.write("\n\n=== GOOGLE GEMINI MATH ANALYSIS (TYPED) ===\n")
            f.write(python_resp_text := response.text)
            f.write("\n===========================================\n\n")
        ...
        print(f"Error calling Gemini AI for typed work: {e}")
        return {
            "is_valid": False,
            "feedback": "There was an error analyzing your typed calculations. Please review your input and try again.",
            "sub_topic": "default"
        }
